...
public class RattleOfCybeleItem extends Item implements GeoItem {
    private final Supplier<Object> renderProvider = GeoItem.makeRenderer(this);
    private final AnimatableInstanceCache cache = AzureLibUtil.createInstanceCache(this);
    private boolean isPlaying = false;
    private int COOLDOWN_TICKS = 120 * 20;

    public RattleOfCybeleItem(Properties pProperties) {
        super(pProperties);
    }
        ...
        if (!world.isClientSide()) {
            if (!isMusicPlaying() && !player.getCooldowns().isOnCooldown(this)) {
                ...
                for (Player nearbyPlayer : players) {
                        nearbyPlayer.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 600, 1, false, true, false));
                }
    ...
    private void playCustomSound(Level world, @Nullable LivingEntity entity) {
        if (entity != null) {
            SoundEvent customSound = ModSounds.RATTLE_OF_CYBELE.get();
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if (Screen.hasShiftDown()) {
            pTooltipComponents.add(Component.translatable("tooltip.greekmyth.rattle_of_cybele_shift"));
        } else if (Screen.hasControlDown()){
            pTooltipComponents.add(Component.translatable("tooltip.greekmyth.rattle_of_cybele_ctrl"));
        } else {
            pTooltipComponents.add(Component.translatable("tooltip.greekmyth.rattle_of_cybele"));
        }
