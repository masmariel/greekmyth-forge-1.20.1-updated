...
import net.rafiki.greekmyth.client.CrownOfDionysusRenderer;
import net.rafiki.greekmyth.client.WreathOfNikeRenderer;
...
public class CrownOfDionysusItem extends ModArmorItem implements GeoItem {
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private CrownOfDionysusRenderer renderer = null;

            @Override
            public HumanoidModel<LivingEntity> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot, HumanoidModel<LivingEntity> original) {
                if (renderer == null)
                    renderer = new CrownOfDionysusRenderer();
    ...
    public void inventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected) {
        ...
        if (!pLevel.isClientSide && player.getInventory().armor.get(3) == pStack && pEntity instanceof Player) {
            player.addEffect(new MobEffectInstance(MobEffects.CONFUSION, 119, 0, false, true, false));
        }
    }
