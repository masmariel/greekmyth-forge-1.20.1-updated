...
import net.rafiki.greekmyth.client.LyreOfApolloRenderer;
...
public class LyreOfApolloItem extends Item implements GeoItem {
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private LyreOfApolloRenderer renderer = null;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (renderer == null)
                    return new LyreOfApolloRenderer();
        ...
        if (!world.isClientSide()) {
            if (!isMusicPlaying() && !player.getCooldowns().isOnCooldown(this)) {
                ...
                for (Player nearbyPlayer : players) {
                    nearbyPlayer.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 600, 1, false, true, false));
                }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if (Screen.hasShiftDown()) {
            ...
            pTooltipComponents.add(Component.translatable("tooltip.greekmyth.lyre_of_apollo_ctrl"));
        } else {
