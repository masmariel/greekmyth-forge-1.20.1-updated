...
import net.rafiki.greekmyth.client.TrumpetOfPhemeRenderer;
...
public class TrumpetOfPhemeItem extends Item implements GeoItem {
    private final Supplier<Object> renderProvider = GeoItem.makeRenderer(this);
    private final AnimatableInstanceCache cache = AzureLibUtil.createInstanceCache(this);
    private boolean isPlaying = false;
    private int COOLDOWN_TICKS = 150 * 20;

    public TrumpetOfPhemeItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private TrumpetOfPhemeRenderer renderer = null;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (renderer == null)
                    return new TrumpetOfPhemeRenderer();
        ...
        if (!world.isClientSide()) {
            if (!isMusicPlaying() && !player.getCooldowns().isOnCooldown(this)) {
                ...
                for (Player nearbyPlayer : players) {
                    nearbyPlayer.addEffect(new MobEffectInstance(MobEffects.HEALTH_BOOST, 600, 0, false, true, false));
                }

                if (world instanceof ServerLevel) {
                    ServerLevel serverWorld = (ServerLevel) world;
                    ParticleOptions particle = ParticleTypes.SCRAPE;
    ...
    private void playCustomSound(Level world, @Nullable LivingEntity entity) {
        if (entity != null) {
            SoundEvent customSound = ModSounds.TRUMPET_OF_PHEME.get();
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        if (Screen.hasShiftDown()) {
            pTooltipComponents.add(Component.translatable("tooltip.greekmyth.trumpet_of_pheme_shift"));
        } else if (Screen.hasControlDown()){
            pTooltipComponents.add(Component.translatable("tooltip.greekmyth.trumpet_of_pheme_ctrl"));
        } else {
            pTooltipComponents.add(Component.translatable("tooltip.greekmyth.trumpet_of_pheme"));
        }
