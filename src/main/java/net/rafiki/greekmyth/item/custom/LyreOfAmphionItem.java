...
public class LyreOfAmphionItem extends Item implements GeoItem {
    private final Supplier<Object> renderProvider = GeoItem.makeRenderer(this);
    private final AnimatableInstanceCache cache = AzureLibUtil.createInstanceCache(this);
    private boolean isPlaying = false;
    private int COOLDOWN_TICKS = 150 * 20;

    public LyreOfAmphionItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public void createRenderer(Consumer<Object> consumer) {
        consumer.accept(new RenderProvider() {
            private LyreOfAmphionRenderer renderer = null;

            @Override
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (renderer == null)
                    return new LyreOfAmphionRenderer();
            ...
            if (!isMusicPlaying() && !player.getCooldowns().isOnCooldown(this)) {
                ...
                for (LivingEntity nearbyEntity : entities) {
                    if (!(nearbyEntity instanceof Player)) {
                        nearbyEntity.addEffect(new MobEffectInstance(ModEffects.BLEEDING.get(), 600, 0, false, true, false));
                    }
                }
    ...
    private void playCustomSound(Level world, @Nullable LivingEntity entity) {
        if (entity != null) {
            SoundEvent customSound = ModSounds.LYRE_OF_AMPHION_MELODY.get();
