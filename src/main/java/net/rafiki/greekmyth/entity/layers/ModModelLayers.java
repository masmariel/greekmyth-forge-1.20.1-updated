package net.rafiki.greekmyth.entity.layers;

import net.minecraft.client.model.geom.ModelLayerLocation;
import net.minecraft.resources.ResourceLocation;
import net.rafiki.greekmyth.GreekMyth;

public class ModModelLayers {
    public static final ModelLayerLocation MEDUSA_LAYER = new ModelLayerLocation(
            new ResourceLocation(GreekMyth.MOD_ID, "medusa_layer"), "medusa_layer");
    public static final ModelLayerLocation CALYDONIAN_BOAR_LAYER = new ModelLayerLocation(
            new ResourceLocation(GreekMyth.MOD_ID, "calydonian_boar_layer"), "calydonian_boar_layer");
    public static final ModelLayerLocation ERYMANTHIAN_BOAR_LAYER = new ModelLayerLocation(
            new ResourceLocation(GreekMyth.MOD_ID, "erymanthian_boar_layer"), "erymanthian_boar_layer");
}
